...
export class DynamicTerrain {
    ...
    constructor(
        ...
        this._mapSizeX = Math.abs(
            mapData[(this._mapSubX - 1) * 3] - mapData[0]
        );
        ...
        const ribbonOptions = {
            ...
            sideOrientation: options.invertSide
                ? Mesh.FRONTSIDE
                : Mesh.BACKSIDE,
            colors: terrainColor,
            uvs: terrainUV,
    ...
    public update(force: boolean): DynamicTerrain {
        ...
        this._cameraLODCorrection =
            this.updateCameraLOD(this._terrainCamera) | 0;
        ...
        this.centerLocal.copyFromFloats(
            terrainHalfSizeX,
            0.0,
            terrainHalfSizeZ
        );
    ...
    private _updateTerrain(updateSize: boolean): void {
        ...
        const composeToRef = DynamicTerrain._ComposeToRef;
        const copyArrayValuesFromToRef =
            DynamicTerrain._CopyArrayValuesFromToRef;
        ...
        for (let j = 1 | 0; j <= terrainSub; j++) {
            ...
            for (l = 0; l < LODLimits.length; l++) {
                ...
                if (
                    (LODngtvZ && j < LODLimitDown) ||
                    (LODpstvZ && j > LODLimitUp)
                ) {
                    axisZLODValue = l + 1 + LODValue;
                }
                if (
                    (LODngtvX && j < LODLimitDown) ||
                    (LODpstvX && j > LODLimitUp)
                ) {
        ...
        for (let j = 0 | 0; j <= terrainSub; j++) {
            ...
            for (let i = 0 | 0; i <= terrainSub; i++) {
                ...
                for (l = 0; l < LODLimits.length; l++) {
                    ...
                    LODLimitUp = terrainSub - LODLimitDown - 1;
                    if (
                        (LODngtvX && i < LODLimitDown) ||
                        (LODpstvX && i > LODLimitUp)
                    ) {
                        axisLODValue = l + 1 + LODValue;
                    }
                    lodI = axisLODValue;
                    ...
                    if (quads[index]) {
                        ...
                        for (let t = 0; t < quad.length; t++) {
                            ...
                            if (partIndexes) {
                                ...
                                let min =
                                    available < nbQuadParticles
                                        ? available
                                        : nbQuadParticles; // don't iterate beyond possible
                                for (let pIdx = 0; pIdx < min; pIdx++) {
                                    ...
                                    const particle =
                                        particles[typeStartIndex + pIdx + used];
                ...
                if (instanceMap) {
                    ...
                    if (quads[index]) {
                        ...
                        for (let t = 0; t < quad.length; t++) {
                            ...
                            const instances = sourceMesh.instances;
                            const instancedBuffer =
                                sourceMesh.worldMatrixInstancedBuffer;
                            const data = instanceMapData[t];
                            ...
                            if (instanceIndexes && instancedBuffer) {
                                ...
                                var used = rem > 0 ? rem : 0;
                                let min =
                                    available < nbQuadInstances
                                        ? available
                                        : nbQuadInstances; // don't iterate beyond possible
                                ...
                                available = available > 0 ? available : 0;
                                this._nbAvailableInstancesPerType[t] =
                                    available;
                            }
                        }
                    }
        ...
        if (instanceMap && nbAvailableInstancesPerType) {
            for (let c = 0; c < nbAvailableInstancesPerType.length; c++) {
                nbAvailableInstancesPerType[c] =
                    this._sourceMeshes[c].instances.length;
            }
        }
        ...
        terrain
            .getBoundingInfo()
            .reConstruct(bbMin, bbMax, terrain._worldMatrix);
    }
    ...
    public set mapData(val: Float32Array | number[]) {
        ...
        this._mapSizeZ = Math.abs(
            val[(mapSubZ - 1) * mapSubX * 3 + 2] - val[2]
        );
